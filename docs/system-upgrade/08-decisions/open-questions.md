...
| Q-HD-10 | **BatchTask schema is FRONTEND-DEFINED.** `lib/modules/helpdesk/types.ts BatchTask` shipped 2026-05-03 with `lib/api/helpdesk.batch.ts` mock client. Legacy `BatchTask` model existed (apps/helpdesk legacy stack) but is not yet ported. Frontend assumes a job-queue worker (Celery/RQ) on the backend writing progress + result. The `task_type` is an open string (`bulk_reassign \| sla_recompute \| bulk_export \| ticket_archive \| string & {}`) — backend is free to add new types but the existing five MUST emit identical type slugs. | Same drift class as Q-HD-9. Additionally: progress polling pattern (current 5s `refetchInterval`) needs backend support — long-poll, server-sent events, or cheap GET. Decide before R042-BE-min ships. | Decide: (a) backend adopts frontend shape verbatim and exposes `GET /api/helpdesk/api/batch/:id` for cheap polling; (b) backend uses different shape, frontend adds `transformFlaskBatchTask()`. **Recommendation: (a)** + plan SSE/websocket as a follow-up only if polling proves expensive. |
