...
#   7. i18n orphan gate — fails if any i18n catalog leaf has no consumer
#      reference (batch 127; locks the 0-orphan state).
#   8. next build — catches latent prerender bugs (e.g. missing
...
step "7/8 i18n orphan gate"
node scripts/audit-i18n-orphans.mjs --gate || fail "i18n orphan gate failed"
ok "i18n orphan gate clean (0 unused leaves)"
